...
module.exports = async (req, res) => {
  ...
  try {
    ...
    console.error('Actives Tier1 (Yahoo) failed:', e.message);
  }
  ...
  try {
    ...
    setCached(cacheKey, actives, 900);
    res.setHeader('X-Cache', 'MISS');
    res.setHeader('X-Source', 'alphavantage');
    return res.json(actives);
  } catch (e) {
    console.error('Actives Tier2 (AlphaVantage) failed:', e.message);
  }

  res.status(500).json({ error: 'All actives sources failed' });
};
